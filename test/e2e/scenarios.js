...
describe('conFusion App E2E Testing', function() {
	...
	describe('menu 0 item', function() {
		...
		it('should show the first comment author as', function() {
			...
			/* note: the following 'comment in dish.comments' is in the order of 'sort' 
			$scope.sort should be initialized to "", otherwise sendKeys() will append 
			extra string to it */
			element(by.model('sort')).sendKeys('author');

			// without a html element with {{sort}}, by.binding('sort') doesn't work
			//var sort = element(by.binding('sort'));
			//expect(sort.getText()).toContain('abc');
			...
			//expect(author.getText()).toContain('abc');
		});
	});
